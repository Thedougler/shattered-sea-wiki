...
1. Update `wiki/situations/active/` index if any threads resolved, escalated, or
   changed status
2. Refresh `wiki/hot.md` with the new world state
3. Commit: `git add wiki/ && git commit -m "world-tick: [session number or date] offscreen advancement"`
