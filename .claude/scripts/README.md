...
python3 .claude/scripts/tag_taxonomy.py           # controlled tag vocabulary (data module; consumed by sea lint)
python3 .claude/scripts/check_taxonomy_sync.py    # verify tag_taxonomy.py ↔ wiki/system/taxonomy.md stay in sync
sea lint                                          # lint vault (see sea lint --help for flags)
sea health                                        # capture vault health metrics (--save to persist)
```
