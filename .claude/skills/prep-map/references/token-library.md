...
The token manifest (`tokens/tokens.json`) is the single source of truth. Each token serves **four consumers**: the agent (the short CSV code), the renderer (a distinct flat `color` + an optional `glyph` SVG), and the img2img beautify pass (the `fragment` phrase, assembled into the auto color→material legend). This file is generated from the manifest — do not hand-edit.
...
**Palette gate.** Large flat regions (terrain/structure) stay ≥16 ΔE2000 apart; small glyph-bearing objects (features/markers) stay ≥10. Enforced by `palette.checkSeparation` as a unit test.
