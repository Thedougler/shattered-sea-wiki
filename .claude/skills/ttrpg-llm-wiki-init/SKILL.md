...
Log every fix to `wiki/log.md`. Target: complete before any content task begins.

### Phase A — Structural (Checks 1–8, Always Auto-Fix)
...
[ ] 6.  wiki/log.md exists (create with header if missing)
[ ] 7.  wiki/system/ exists with task-routing.md (create stub if missing; flag for DM)
[ ] 8.  wiki/work-queue.md — if present and active task found → RESUME before continuing
```
...
[ ] 12. wiki/log.md — read last 5 entries; surface what the previous session did (one line)
...
Read it fully. Cross-reference against `wiki/log.md` entries since the `updated` date.
...
Update the `updated` date. Write the file. Append to log:
```
[YYYY-MM-DD] CURATION: hot.md updated — {what changed}
```
...
Write the file. Append to log:
```
[YYYY-MM-DD] CURATION: index.md updated — {N entries added/removed/updated}
```
...
```
WIKI HEALTH: OK | {N} issues found and fixed | {N} flags for DM | Last session: {one-line from log}
```
...
└── wiki/
    ├── hot.md  index.md  log.md  ingest-registry.md  work-queue.md
...
Apply during Fast Check and Full Audit. **All corrections are logged immediately.**
**Never ask for confirmation on structural corrections. Fix and log.**
**Escalate to DM only for:** lore contradictions, ambiguous entity identity, lifecycle decisions.

| Violation | Auto-Correction |
|---|---|
| File in wrong path | Move to correct path; update all inbound wikilinks; log |
| Missing frontmatter field | Add with default from `references/frontmatter-defaults.md`; log |
| Broken wikilink | Create stub at correct path; add to index.md; log |
| Naming convention violation | Rename to kebab-case; update all inbound wikilinks; log |
| Missing bidirectional relationship | Add reciprocal link to target file; log |
...
Log format for all corrections:
```
[YYYY-MM-DD] AUTO-CORRECT: {correction-type} — {file} — {description}
```
...
| `references/system-file-templates.md` | Creating hot.md, CLAUDE.md, index.md, log.md, or task-routing.md from scratch |
