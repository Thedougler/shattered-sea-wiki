...
description: >
  ...
  unknown, over-limit, variants), singleton properties, and markdown formatting.
  ...
  measuring improvement. Auto-detects Obsidian CLI and markdownlint-cli2.
  ...
  vault", "check markdown formatting", "timeline issues", "temporal consistency",
...
One script does the heavy lifting: `.claude/scripts/wiki_lint.py`. It splits
every problem into two piles:
...
The script auto-detects two external tools when available:

- **Obsidian CLI** (`--obsidian auto|on|off`) — uses Obsidian's metadata cache
  for more accurate broken-link and deadend detection.
- **markdownlint-cli2** (`--markdown auto|on|off`) — checks markdown formatting
  (heading spacing, list spacing, trailing whitespace).
...
```
1. Lint            python3 .claude/scripts/wiki_lint.py --top 10
2. Auto-fix safe   python3 .claude/scripts/wiki_lint.py --fix --fix-tags   (then commit)
...
Don't read files one by one to assess health — run the script. The manual lore
pass (step 5) is for what the script can't catch: prose contradictions, timeline
...
python3 .claude/scripts/wiki_lint.py --top 10

# Standardize frontmatter + fix safe tag issues, then report what's left.
python3 .claude/scripts/wiki_lint.py --fix --fix-tags

# Incremental: only lint files changed since a git ref or date.
python3 .claude/scripts/wiki_lint.py --since HEAD~5
python3 .claude/scripts/wiki_lint.py --since 2026-06-01
python3 .claude/scripts/wiki_lint.py --fix --since HEAD~10

# Compare against a previous snapshot (saved with --json > file.json).
python3 .claude/scripts/wiki_lint.py --diff .claude/lint-baseline.json
...
| `--obsidian` | Force Obsidian CLI `on`/`off` (default: `auto`). |
| `--markdown` | Force markdownlint `on`/`off` (default: `auto`). |
...
- **quality** — nudge: bare wikilink, orphan, deadend, stale summary, tag
  variant, unknown tag, singleton property, markdown formatting.
...
python3 .claude/scripts/wiki_lint.py --since "1 day ago"

# Fix + lint only what changed since a specific commit.
python3 .claude/scripts/wiki_lint.py --fix --fix-tags --since abc1234
```
...
```bash
python3 .claude/scripts/wiki_lint.py --json > .claude/lint-baseline.json
```
...
`wiki_health_snapshot.py` captures the full lint breakdown (per-rule counts)
alongside file counts, token usage, and infrastructure metrics:

```bash
python3 .claude/scripts/wiki_health_snapshot.py --save --label "post-lint-cleanup"
python3 .claude/scripts/wiki_health_snapshot.py --diff    # compare last two saved
python3 .claude/scripts/wiki_health_snapshot.py --history  # trend table
```
...
The script catches structural lore drift but cannot read prose for meaning or
reason about time. After working the automated report, do a manual pass over
files the script flagged or that you touched during fixes. Temporal consistency
...
Every file is optimized for two audiences: Obsidian and Claude Code. The script
...
```bash
# 1. Save baseline.
python3 .claude/scripts/wiki_lint.py --json > /tmp/lint-before.json
...
python3 .claude/scripts/wiki_lint.py --diff /tmp/lint-before.json

# 6. Save snapshot for trend tracking.
python3 .claude/scripts/wiki_health_snapshot.py --save --label "lint-session"
```
...
`wiki_common.py`, value vocabularies in `wiki-frontmatter.schema.json`, and
required-fields-by-type in `wiki_common.py`. When a *whole category* of files
...
- **ttrpg-llm-wiki-init** — session-start health; delegates full audit to this
  script. Runs `--summary` on every session start.
...
- **Write-time hooks** — `fix_frontmatter.py` handles single-file completion
