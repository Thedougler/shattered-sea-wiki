...
def _build_nav_bar():
    with ui.card().classes("dm-card w-full").style("padding: 10px 16px"):
        ...
        def refresh():
            nav_row.clear()
            with nav_row:
                ui.label("PLAYER VIEWS").classes("dm-section-label").style(
                    "margin-right: 4px"
                )
                ...
                if map_names:
                    ui.separator().props("vertical").classes("self-stretch").style(
                        "margin: 0 4px"
                    )
                    for name in map_names:
                        ui.link(f"Map: {name}", f"/map/{name}", new_tab=True).classes(
                            "nav-pill"
                        )
...
def _build_status_bar():
    with ui.card().classes("dm-card w-full").style("padding: 8px 16px"):
        ...
        def refresh():
            status_row.clear()
            with status_row:
                ui.label("STATUS").classes("dm-section-label").style(
                    "margin-right: 4px"
                )

                profile_count = (
                    len(services.profiles.load_all()) if services.profiles else 0
                )
                ...
                sess_status = (
                    services.session_state.status if services.session_state else "idle"
                )
    ...
    with ui.card().classes("dm-card flex-1"):
        ...
        def on_dest_change():
            ...
            if is_avatar:
                ...
                char_select.options = [
                    p.character for p in sorted(all_profiles, key=lambda x: x.character)
                ]
                char_select.update()
        ...
        def on_upload(e: events.UploadEventArguments):
            ...
            if dest == "Maps":
                ...
                info_lbl.text = (
                    f"Slide added — {len(slideshow_page._state.images)} total"
                )
            elif dest == "Avatar":
                ...
                with results_col:
                    for name, sim in ranked[:5]:
                        ...
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.linear_progress(value=sim).classes("w-32").props(
                                f'color="{color}"'
                            )
                            ui.label(f"{pct}%").style(
                                f"color: {color}; font-size: 0.8rem"
                            )
            ...
            with profiles_row:
                for p in sorted(all_profiles, key=lambda x: x.character):
                    ...
                    with (
                        ui.card()
                        .classes("p-2")
                        .style("background: #1a1a3e; border: 1px solid #333")
                    ):
                        with ui.row().classes("items-center gap-2"):
                            if avatar_url:
                            ...
                            ui.button(
                                icon="photo_camera",
                                on_click=lambda _, n=p.character: _show_avatar_upload(
                                    n
                                ),
...
def _build_session_card():
    ...
    with ui.card().classes("dm-card flex-1"):
        ...
        with ui.row().classes("gap-2 items-center"):
            dm_ch = (
                ui.input("DM ch", value="0")
                .props("dense outlined dark")
                .classes("w-16")
            )
            player_ch = (
                ui.input("Player ch", value="1,2")
                .props("dense outlined dark")
                .classes("w-20")
            )
        ...
        def poll():
            ...
            if state["active"] and state.get("session_audio"):
                ...
                if len(levels) >= 2:
                    ratio_lbl.text = (
                        f"DM: {levels[0]:.0%} | Player: {max(levels[1:]):.0%}"
                    )
...
def _build_slideshow_card():
    with ui.card().classes("dm-card flex-1"):
        ...
        with ui.row().classes("gap-2 items-center"):
            interval_num = (
                ui.number("Interval (s)", value=10, min=1, max=120)
                .props("dense outlined dark")
                .classes("w-28")
            )
        ...
        def on_upload(e: events.UploadEventArguments):
            ...
            if path_str not in slideshow_page._state.images:
                slideshow_page._state.images.append(path_str)
            info_lbl.text = (
                f"uploaded {e.name} — {len(slideshow_page._state.images)} total"
            )
            e.sender.run_method("reset")
            ...
            if names:
                with thumbs_row:
                    for name in names:
                        ...
                        with (
                            ui.column()
                            .classes("items-center gap-1 map-thumb")
                            .style("padding: 4px")
                        ):
                            ui.image(src).style(
