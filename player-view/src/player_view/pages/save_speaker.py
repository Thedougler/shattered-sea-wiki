...
def save_speaker_page():
    ...
    with operator_header("Save Speaker"):
        ...
        name_input = (
            ui.input("Name", on_change=_on_name_change).props("dense").classes("w-36")
        )
        record_btn = ui.button("REC", on_click=lambda: start(), color="red").props(
            "dense"
        )
        stop_btn = ui.button(
            "STOP & SAVE", on_click=lambda: stop(), color="green"
        ).props("dense")
    ...
    with ui.row().classes("status-strip"):
        ...
        ui.html('<div class="vu-meter"><div id="vu-fill" class="vu-fill"></div></div>')
    ...
    def poll_asr():
        ...
        if result.finalized_text or result.draft_text:
            print(
                f"[ASR] finalized={result.finalized_text!r} draft={result.draft_text!r}"
            )
    ...
    def poll_vu():
        ...
        ui.run_javascript(f'document.getElementById("vu-fill").style.width="{pct}%"')
