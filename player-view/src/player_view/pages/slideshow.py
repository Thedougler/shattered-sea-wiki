...
def slideshow_page():
    ...
    with operator_header("Slideshow"):
        count_lbl = ui.label(f"{len(_state.images)} images").style("color: #888")  # noqa: F841
        index_lbl = ui.label("0").style("color: #888")
        interval_lbl = ui.label(f"{_state.interval_s}s").style("color: #888")  # noqa: F841

    ui.query("body").style("overflow: hidden")

    if _state.images:
        img = (
            ui.image(_state.images[0])
            .classes("w-full")
