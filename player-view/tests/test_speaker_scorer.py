...
class TestSpatialAwareRanking:
    ...
    def test_rank_without_spatial_matches_original(self, diarization):
        ...
        original = diarization.rank_against(target, [(n, e) for n, e, *_ in profiles])
...
class TestAdaptiveWeight:
    ...
    def test_few_samples_low_weight(self, diarization):
        ...
        current = np.array([0.72, 0.14, 0.14])
        r_few = diarization.rank_against_with_spatial(
            emb_norm, current, profiles_few, base_weight=0.3
        )
        r_many = diarization.rank_against_with_spatial(
            emb_norm, current, profiles_many, base_weight=0.3
        )
    ...
    def test_high_consistency_increases_spatial_influence(self, diarization):
        ...
        current = np.array([0.72, 0.14, 0.14])
        r_consistent = diarization.rank_against_with_spatial(
            emb_norm, current, profiles_consistent, base_weight=0.3
        )
        r_noisy = diarization.rank_against_with_spatial(
            emb_norm, current, profiles_noisy, base_weight=0.3
        )
