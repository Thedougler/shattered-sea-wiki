...
class TestProfileStore:
    ...
    def test_load_all(self, profile_store):
        for name in ["Alpha", "Beta", "Gamma"]:
            emb = np.random.randn(192).astype(np.float32)
            profile_store.save(
                VoiceProfile(character=name, player="test", embedding=emb)
            )
    ...
    def test_delete(self, profile_store):
        emb = np.random.randn(192).astype(np.float32)
        profile_store.save(
            VoiceProfile(character="ToDelete", player="test", embedding=emb)
        )
    ...
    def test_update_embedding(self, profile_store):
        ...
        profile_store.save(
            VoiceProfile(character="Updater", player="test", embedding=emb1)
        )
...
class TestSpatialFingerprint:
    ...
    def test_fingerprint_serialization_roundtrip(self):
        ...
        profile = VoiceProfile(
            ...
            player="Test",
            embedding=emb,
            spatial_fingerprint=fp,
            spatial_m2=m2,
            spatial_sample_count=10,
        )
        d = profile.to_dict()
        restored = VoiceProfile.from_dict(d)
