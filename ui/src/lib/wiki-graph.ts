...
import { deriveSlug, slugify, titleCase } from './slug-utils';

let slugSetCache: Set<string> | null = null;

export async function buildSlugSet(): Promise<Set<string>> {
  if (slugSetCache) return slugSetCache;

  const wikiDir = path.resolve(import.meta.dirname, '../../../wiki');
  const files = await fg('**/*.md', { cwd: wikiDir });
...
export async function buildBacklinks(): Promise<Map<string, BacklinkEntry[]>> {
  const wikiDir = path.resolve(import.meta.dirname, '../../../wiki');
  const files = await fg('**/*.md', { cwd: wikiDir });
