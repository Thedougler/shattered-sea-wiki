...
## Encounter Adaptation — Catarina's Kit (and Staggered-Entry Fallback)

[[catarina-davirelli|Catarina]] is introduced earlier in Session 06 (run guide Scene 3) and normally joins the party **before** the primary chamber — so by default she is a full 5th PC here, in from turn 1. The staggered entry below is the **fallback**: run it only if the party reaches Room 8 before the DM has introduced her, in which case two pieces of her kit are on site and active from round 1 while she is 2–3 turns of movement out. Either way, the cannon/owl stats below are the canonical numbers.

> [!mechanic]
> **Ragnetto — the Eldritch Cannon (deploys ahead of her).** Catarina's spider-walking "cannon backpack" — she sends it in rather than carrying it. Tiny construct, AC 18, HP 25, immune to poison/psychic, can't be charmed/frightened/paralyzed. Set to **Force Ballista** (ranged spell attack, 60 ft, 2d8 force; use Catarina's Int mod + proficiency, ≈ +7 — confirm against her sheet). She commands it with a bonus action; it can swap to **Protector** (temp HP) if the party bleeds. **Never targets [[jean-claude-tabarnack|Jean-Claude]].** *Staggered-entry fallback only:* while unattended (she's not yet within 60 ft), it defaults to firing at the nearest hostile it can see each round, no command needed.
>
> **Strix — the clockwork owl (scouting).** [[clockwork-owl-familiar|Clockwork Owl Familiar]] — use the Owl stat block (Tiny beast/construct, AC 11, HP 1, flying 60 ft, Flyby, Keen Hearing and Sight). It scouts ahead and watches the crater. At 1 HP it dies to almost anything that touches it — Chaos Pulse's splash radius or a stray Claw ends it instantly. It can Help a PC (advantage on their next attack against a creature within 5 ft of the owl) or feed Catarina's player real-time intel. **Table beat:** if Otar's AoE catches it, that's a legitimate gut-punch moment, not a rules problem — let it land.
>
