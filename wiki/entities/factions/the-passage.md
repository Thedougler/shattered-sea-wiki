---
type: entity
subtype: faction
campaign: shattered-sea
status: active
audience: dm
publish: false
summary: "Rattkin network of Runs, Holds, caches, couriers, and hidden infrastructure beneath and between Shattered Sea ports. Nona Black-Jaw's Black-Jaw Run anchors the Calveno Warren."
created: 2026-05-27
updated: 2026-05-28
tags: [faction, rattkin, passage]
sources: ["Inbox/Session-02-Recap.md", "Inbox/Session-03-Recap.md", "Inbox/Campaign-Timeline.md", "Inbox/Rattkin.md", "Inbox/Demographics.md", "Inbox/Passage.md", "Inbox/Black-Jaw-Run.md"]
confidence_level: inferred
relationships:
  - "[[perrin-black-jaw|Perrin Black-Jaw]] — Friend of the Passage (granted by Beaumont)"
  - "[[beaumont-sel|Beaumont Sel]] — granted Passage status to Perrin"
  - "[[nona-black-jaw|Nona Black-Jaw]] — operates within"
  - "[[warren|Warren]] — network origin"
  - "[[black-jaw-run|Black-Jaw Run]] — Calveno anchor Run"
  - "[[dravosi-crown|Dravosi Crown]] — bounties and colonial pressure"
  - "[[grung-clans|Grung Clans]] — caution because of bondage and coastal raids"
---

# The Passage

A rattkin network formed from [[warren|the Warren]] in 1240 DR to evade colonial pressure from the [[tessarine-concordat|Tessarine Concordat]] and [[dravosi-crown|Dravosi Crown]].

"Friend of the Passage — any Kin in need can ask." [[beaumont-sel|Beaumont]] granted [[perrin-black-jaw|Perrin]] this status after the Saltwright boarding.

See [[rattkin|Rattkin]] for species traits, varieties, and in-group terms. In Passage vocabulary, **Kin** means rattkin, **Dry** means non-rattkin, a **Run** is a local port network, and a **Hold** is a waypoint or safe house.

The Passage moves people, cargo, and information across routes that do not appear on colonial charts. Its knowledge is institutional: which harbor wall has a crawlspace, which dock beam carries a warning mark, which clerk can be relied on, which tide opens a low tunnel, which names carry risk in a port.

## Structure

- **The Run** — travelling families and crews whose circuit knowledge passes through generations, encoded in scratch-notation on bone and ceramic shards legible only to Kin.
- **The Hold** — caches and shelters inside drainage channels, dock supports, wall cavities, old foundations, roof voids, cisterns, and forgotten city spaces.
- **The Tangle** — seven senior elders governing the Calveno Run and the Passage's most sensitive routing intelligence.

[[nona-black-jaw|Nona Black-Jaw]] leads the [[black-jaw-run|Black-Jaw Run]], the Warren's anchor Run. [[enzo|Enzo]] executes her directives. [[ruk|Ruk]] handles kitchen security. [[cobb|Cobb]] operates the [[la-vasca|La Vasca]] dry dock under Black-Jaw colors.

## One Law

The Passage does not move people in bondage. Cargo can be hidden, messages carried, and people smuggled to freedom. Enslaved people are not a trade good the Passage will handle.

## Session 03 Update

Nona called off the attacks on Perrin when he arrived in her kitchen — she'd believed he was captured. [[anzolo|Anzolo]] was already in motion before the directive reached him; his status is unconfirmed. Perrin owes Nona a favor (terms unstated).

> [!dm]
> The organizational structure — how many cells, what geographic reach, who sits above Nona — is not established. Expand before the network becomes plot-relevant.
